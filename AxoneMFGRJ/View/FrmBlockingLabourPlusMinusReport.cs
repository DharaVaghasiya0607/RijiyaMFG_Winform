...
{
    public partial class FrmBlockingLabourPlusMinusReport : Form
    {
        ...
        public FrmBlockingLabourPlusMinusReport()
        {
            InitializeComponent();
        }
        ...
        private void BtnRefresh_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataSet DS = Obj.GetBlockingDollarPlusMinusProductionLabour(StrOpe, Val.Trim(CmbKapan.Properties.GetCheckedItems()), Val.ToInt(txtProcess.Tag), Val.ToInt64(txtEmployee.Tag), Val.SqlDate(DTPFromDate.Value.ToShortDateString()), Val.SqlDate(DTPToDate.Value.ToShortDateString()));
        ...
        private void toolTipController1_GetActiveObjectInfo(object sender, DevExpress.Utils.ToolTipControllerGetActiveObjectInfoEventArgs e)
        {
            ...
            try
            {
                ...
                if (hi.HitTest == GridHitTest.RowCell && hi.Column.FieldName == "MKBLWORKERCODE")
                {
                    info = new ToolTipControlInfo(hi.RowHandle.ToString() + hi.Column.FieldName, Val.ToString(view.GetRowCellValue(hi.RowHandle, "MKBLWORKERNAME")));
                    return;
                }
                if (hi.HitTest == GridHitTest.RowCell && hi.Column.FieldName == "RMKBLWORKERCODE")
                {
                    info = new ToolTipControlInfo(hi.RowHandle.ToString() + hi.Column.FieldName, Val.ToString(view.GetRowCellValue(hi.RowHandle, "RMKBLWORKERNAME")));
                    return;
                }
            }
            finally
        ...
        private void GrdDet_RowCellStyle(object sender, RowCellStyleEventArgs e)
        {
            ...
            if (e.Column.FieldName == "RMKBLSHAPENAME")
            {
                string StrMakShape = Val.ToString(GrdDet.GetRowCellValue(e.RowHandle, "MKBLSHAPENAME"));
                string StrShape = Val.ToString(GrdDet.GetRowCellValue(e.RowHandle, "RMKBLSHAPENAME"));
            ...
            if (e.Column.FieldName == "RMKBLCLARITYNAME")
            {
                int IntMakSeqNo = Val.ToInt(GrdDet.GetRowCellValue(e.RowHandle, "MKBLCLARITYSEQNO"));
                int IntSeqNo = Val.ToInt(GrdDet.GetRowCellValue(e.RowHandle, "RMKBLCLARITYSEQNO"));
            ...
            if (e.Column.FieldName == "RMKBLCUTNAME")
            {
                int IntMakSeqNo = Val.ToInt(GrdDet.GetRowCellValue(e.RowHandle, "MKBLCUTSEQNO"));
                int IntSeqNo = Val.ToInt(GrdDet.GetRowCellValue(e.RowHandle, "RMKBLCUTSEQNO"));
                if (IntSeqNo != 0 && IntMakSeqNo != 0 && IntSeqNo > IntMakSeqNo)
                {
                    e.Appearance.BackColor = lblUp.BackColor;
                }
            ...
            if (e.Column.FieldName == "RMKBLPOLNAME")
            {
                int IntMakSeqNo = Val.ToInt(GrdDet.GetRowCellValue(e.RowHandle, "MKBLPOLSEQNO"));
                int IntSeqNo = Val.ToInt(GrdDet.GetRowCellValue(e.RowHandle, "RMKBLPOLSEQNO"));
                if (IntSeqNo != 0 && IntMakSeqNo != 0 && IntSeqNo > IntMakSeqNo)
                {
                    e.Appearance.BackColor = lblUp.BackColor;
                }
            ...
            if (e.Column.FieldName == "RMKBLSYMNAME")
            {
                int IntMakSeqNo = Val.ToInt(GrdDet.GetRowCellValue(e.RowHandle, "MKBLSYMSEQNO"));
                int IntSeqNo = Val.ToInt(GrdDet.GetRowCellValue(e.RowHandle, "RMKBLSYMSEQNO"));
                if (IntSeqNo != 0 && IntMakSeqNo != 0 && IntSeqNo > IntMakSeqNo)
                {
                    e.Appearance.BackColor = lblUp.BackColor;
                }
            ...
            if (e.Column.FieldName == "RMKBLCARAT")
            {
                double DouMakCarat = Val.Val(GrdDet.GetRowCellValue(e.RowHandle, "MKBLCARAT"));
                double DouCarat = Val.Val(GrdDet.GetRowCellValue(e.RowHandle, "RMKBLCARAT"));
            ...
            if (e.Column.FieldName == "RMKBLAMOUNT")
            {
                double DouMakCarat = Val.Val(GrdDet.GetRowCellValue(e.RowHandle, "MKBLAMOUNT"));
                double DouCarat = Val.Val(GrdDet.GetRowCellValue(e.RowHandle, "RMKBLAMOUNT"));
                if (DouCarat != 0 && DouMakCarat != 0 && DouCarat > DouMakCarat)
                {
                    e.Appearance.BackColor = lblDown.BackColor;
                }
        ...
        private void GrdDetSummary_RowCellClick(object sender, RowCellClickEventArgs e)
        {
            ...
            {
                this.Cursor = Cursors.WaitCursor;
                GrdDet.Columns["MKBLWORKERCODE"].ClearFilter();
                GrdDet.Columns["MKBLWORKERCODE"].FilterInfo = new DevExpress.XtraGrid.Columns.ColumnFilterInfo("MKBLWORKERCODE='" + Val.ToString(GrdDetSummary.GetRowCellValue(e.RowHandle, "MKBLWORKERCODE")) + "'");
        ...
        private void BtnExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                string Str = txtEmployee.Text.Trim().Length == 0 ? "All Worker" : txtEmployee.Text;
                mStrReportTitle = "Blocking Production Labour Summary (" + Str + ")";
                try
                {
                    SaveFileDialog svDialog = new SaveFileDialog();
                    svDialog.DefaultExt = ".xlsx";
                    svDialog.Title = "Export to Excel";
                    svDialog.FileName = "BlockingLabourSummary.xlsx";
                    svDialog.Filter = "Excel files 97-2003 (*.xls)|*.xls|Excel files 2007(*.xlsx)|*.xlsx|All files (*.*)|*.*";
                    if ((svDialog.ShowDialog() == System.Windows.Forms.DialogResult.OK))
                    {
                        PrintableComponentLinkBase link = new PrintableComponentLinkBase()
                        ...
                        if (Global.Confirm("Do You Want To Open [BlockingLabourSummary.xlsx] ?") == System.Windows.Forms.DialogResult.Yes)
                        {
                            System.Diagnostics.Process.Start(svDialog.FileName, "CMD");
                        }
                    }
            ...
            {
                string Str = txtEmployee.Text.Trim().Length == 0 ? "All Worker" : txtEmployee.Text;
                mStrReportTitle = "Blocking Production Labour Detail (" + Str + ")";
                try
                {
                    SaveFileDialog svDialog = new SaveFileDialog();
                    svDialog.DefaultExt = ".xlsx";
                    svDialog.Title = "Export to Excel";
                    svDialog.FileName = "BlockingLabourDetail.xlsx";
                    svDialog.Filter = "Excel files 97-2003 (*.xls)|*.xls|Excel files 2007(*.xlsx)|*.xlsx|All files (*.*)|*.*";
                    if ((svDialog.ShowDialog() == System.Windows.Forms.DialogResult.OK))
                    {
                        PrintableComponentLinkBase link = new PrintableComponentLinkBase()
                        ...
                        if (Global.Confirm("Do You Want To Open [BlockingLabourDetail.xlsx] ?") == System.Windows.Forms.DialogResult.Yes)
                        {
                            System.Diagnostics.Process.Start(svDialog.FileName, "CMD");
                        }
                    }
