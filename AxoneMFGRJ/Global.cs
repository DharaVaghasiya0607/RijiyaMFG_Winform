    ...
    static class Global
    {
        ...
        static void Main()
        {
            try
            {
                ...
                Config.ConnectionFileName_TransferDB = "Transferconn";  //For Transfer Database 
                Config.ConnectionString_TransferDB = System.IO.File.ReadAllText(Application.StartupPath + "\\Transferconn.txt");
                Config.ConnectionString_TransferDB = Decrypt(Config.ConnectionString_TransferDB);
                Config.ProviderName_TransferDB = "System.Data.SqlClient";
        ...
        public static DataTable GetDataTableFromTxt(string path)
        {
            ...
            dt.Columns.AddRange(new DataColumn[13] { new DataColumn("StoneName"), new DataColumn("RoughWeight"), new DataColumn(""), new DataColumn(""), new DataColumn("Diameter"), new DataColumn("W"), new DataColumn("Pw"), new DataColumn(""), new DataColumn("Shape"), new DataColumn("Cut"), new DataColumn("Clarity"), new DataColumn("Color"), new DataColumn("ColorShade") });
        ...
        public static void BarcodePrintTSC(DataTable DTab, string StrType) //#P : 16-05-2022
        {
            try
            {
                ...
                using (sw)
                {
                    ...
                    {
                        string StrKapanName = "", PacketNo = "", Tag = "", Date = "", Carat = "", MarkerCode = "", StrBarcodeNo = "", StrGroup = "", StrParentTag = "", StrPktCategory = "";
                        ...
                        StrGroup = Val.ToString(DRow["PACKETGRADENAME"]) + "/" + Val.ToString(DRow["PACKETGROUPNAME"]) + "/" + Val.ToString(DRow["PACKETCATEGORYNAME"]);
                        StrParentTag = Val.ToString(DRow["PARENTTAG"]).Trim() == "" ? "" : "(" + Val.ToString(DRow["PARENTTAG"]) + ")";
            ...
            try
            {
                ...
                using (sw)
                {
                    ...
                    foreach (DataRow DRow in DTab.Rows)
                    {
                        ...
                        Group = Val.ToString(DRow["PACKETGROUPNAME"]) +"/" + Val.ToString(DRow["PACKETCATEGORYNAME"]); ;
