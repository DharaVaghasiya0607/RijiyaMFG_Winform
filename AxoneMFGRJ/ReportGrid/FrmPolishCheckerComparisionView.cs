...
{
    public partial class FrmPolishCheckerComparisionView : Form
    {
        ...
        DataTable DtabComparision = new DataTable();
        ...
        public FrmPolishCheckerComparisionView()
        {
            InitializeComponent();
        }
        ...
        public void ShowForm()
        {
            ...
            string Str = new BOTRN_KapanCreate().GetGridLayout(this.Name, GrdTotal.Name);

            if (Str != "")
            {
                byte[] byteArray = Encoding.ASCII.GetBytes(Str);
                MemoryStream stream = new MemoryStream(byteArray);
                GrdTotal.RestoreLayoutFromStream(stream);
        ...
        private void lblDeleteLayoutSummary_Click(object sender, EventArgs e)
        {
            ...
            int IntResYm = new BOTRN_KapanCreate().DeleteGridLayout(this.Name, GrdDetail.Name);
        ...
        private void lblSaveLayoutManagerWise_Click(object sender, EventArgs e)
        {
        ...
        private void lblDeleteLayoutManagerWise_Click(object sender, EventArgs e)
        {
            int IntRes = new BOTRN_KapanCreate().DeleteGridLayout(this.Name, GrdDetail.Name);
            if (IntRes != -1)
            {
        ...
        private void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                DS = Obj.GetPolishCheckerComparisionView(mStrFromDate, mStrToDate, Val.ToInt64(txtEmployee.Tag), "DETAIL", "", "", "", "", "", 0, 0);

                DTabDetail = DS.Tables[0];
                DtabComparision = DS.Tables[1];
        ...
        private void backgroundWorker1_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            try
            {
                ...
                MainGrdTotal.DataSource = DtabComparision;
                GrdTotal.RefreshData();
        ...
        private void GrdTotal_RowClick(object sender, RowClickEventArgs e)
        {
            try
            {
                if (e.RowHandle < 0)
                ...
                DataRow DR = GrdTotal.GetDataRow(e.RowHandle);
                ...
                string mStrTotalClickType = "";

                mStrYear = Val.ToInt32(DR["LYY"]);
                mStrMonth = Val.ToInt32(DR["LMM"]);
                mStrYearMonth = Val.ToString(DR["LYYMMM"]);

                string StrTitleType = Val.ToString(GrdTotal.FocusedColumn.FieldName).ToUpper() == "TOTALPCS" ? "All" : Val.ProperText(GrdTotal.FocusedColumn.FieldName);

                StrTitle = "Grading Comparision Total Stone Detail of : (" + StrTitleType + ")";

                StrExp = GrdTotal.FocusedColumn.FieldName;

                if (GrdTotal.FocusedColumn.FieldName.Contains("COLOR"))
                {
                    StrGrd = "COLOR";
                    StrClick = "COLOR";
                    mStrTotalClickType = "COLOR";
                    if (mStrYearMonth == "TOTAL")
                    {
                        StrGrd = "COLOR";
                        mStrTotalClickType = "TOTAL";
                    }

                }
                else if (GrdTotal.FocusedColumn.FieldName.Contains("CLARITY"))
                {
                    StrGrd = "CLARITY";
                    StrClick = "CLARITY";
                    mStrTotalClickType = "CLARITY";
                    if (mStrYearMonth == "TOTAL")
                    {
                        StrGrd = "CLARITY";
                        mStrTotalClickType = "TOTAL";
                ...
                else if (GrdTotal.FocusedColumn.FieldName.Contains("CUT"))
                {
                    StrGrd = "CUT";
                    StrClick = "CUT";
                    mStrTotalClickType = "CUT";
                    if (mStrYearMonth == "TOTAL")
                    {
                        StrGrd = "CUT";
                        mStrTotalClickType = "TOTAL";
                ...
                else if (GrdTotal.FocusedColumn.FieldName.Contains("POL"))
                {
                    StrGrd = "POL";
                    StrClick = "POL";
                    mStrTotalClickType = "POL";
                    if (mStrYearMonth == "TOTAL")
                    {
                        StrGrd = "POL";
                        mStrTotalClickType = "TOTAL";
                ...
                else if (GrdTotal.FocusedColumn.FieldName.Contains("SYM"))
                {
                    StrGrd = "SYM";
                    StrClick = "SYM";
                    mStrTotalClickType = "SYM";
                    if (mStrYearMonth == "TOTAL")
                    {
                        StrGrd = "SYM";
                        mStrTotalClickType = "TOTAL";
                ...
                else if (GrdTotal.FocusedColumn.FieldName.Contains("FL"))
                {
                    StrGrd = "FL";
                    StrClick = "FL";
                    mStrTotalClickType = "FL";
                    if (mStrYearMonth == "TOTAL")
                    {
                        StrGrd = "FL";
                        mStrTotalClickType = "TOTAL";
                ...
                DS = new BOTRN_RunninPossition().GetPolishCheckerComparisionView(mStrFromDate, mStrToDate, Val.ToInt64(txtEmployee.Tag), "SUMMURY", mStrTotalClickType, StrExp, StrGrd, "", "", mStrMonth, mStrYear);
        ...
        private void BtnSummuryExport_Click(object sender, EventArgs e)
        {
            string Str = txtEmployee.Text.Trim().Length == 0 ? "All Worker" : txtEmployee.Text;
            mStrReportTitle = "Polish Checker Summury (" + Str + ")";
            try
            {
                SaveFileDialog svDialog = new SaveFileDialog();
                svDialog.DefaultExt = ".xlsx";
                svDialog.Title = "Export to Excel";
                svDialog.FileName = "GradingComparisionSummury.xlsx";
                svDialog.Filter = "Excel files 97-2003 (*.xls)|*.xls|Excel files 2007(*.xlsx)|*.xlsx|All files (*.*)|*.*";
                if ((svDialog.ShowDialog() == System.Windows.Forms.DialogResult.OK))
                {
                    PrintableComponentLinkBase link = new PrintableComponentLinkBase()
                    ...
                    if (Global.Confirm("Do You Want To Open [GradingComparision.xlsx] ?") == System.Windows.Forms.DialogResult.Yes)
                    {
                        System.Diagnostics.Process.Start(svDialog.FileName, "CMD");
                    }
                }
        ...
        {
            string Str = txtEmployee.Text.Trim().Length == 0 ? "All Worker" : txtEmployee.Text;
            mStrReportTitle = "Polish Checker Detail (" + Str + ")";
            try
            {
                SaveFileDialog svDialog = new SaveFileDialog();
                svDialog.DefaultExt = ".xlsx";
                svDialog.Title = "Export to Excel";
                svDialog.FileName = "GradingComparision.xlsx";
                svDialog.Filter = "Excel files 97-2003 (*.xls)|*.xls|Excel files 2007(*.xlsx)|*.xlsx|All files (*.*)|*.*";
                if ((svDialog.ShowDialog() == System.Windows.Forms.DialogResult.OK))
                {
                    PrintableComponentLinkBase link = new PrintableComponentLinkBase()
        ...
        {
            Stream str = new System.IO.MemoryStream();
            GrdTotal.SaveLayoutToStream(str);
            str.Seek(0, System.IO.SeekOrigin.Begin);
            StreamReader reader = new StreamReader(str);
            string text = reader.ReadToEnd();
            int IntRes = new BOTRN_KapanCreate().SaveGridLayout(this.Name, GrdTotal.Name, text);

            Stream strYM = new System.IO.MemoryStream();
            GrdDetail.SaveLayoutToStream(strYM);
            strYM.Seek(0, System.IO.SeekOrigin.Begin);
            StreamReader readerYM = new StreamReader(strYM);
            string textYM = readerYM.ReadToEnd();
            int IntResYM = new BOTRN_KapanCreate().SaveGridLayout(this.Name, GrdDetail.Name, textYM);
        ...
        {
            int IntRes = new BOTRN_KapanCreate().DeleteGridLayout(this.Name, GrdDetail.Name);
            int IntResYM = new BOTRN_KapanCreate().DeleteGridLayout(this.Name, GrdTotal.Name);
        ...
        private void GrdDetail_RowCellStyle(object sender, RowCellStyleEventArgs e)
        {
            try
            {
                ...
                string strcol = Val.ToString(e.Column.FieldName);

                if (StrClick == "COLOR" && (Val.ToString(e.Column.FieldName) == "COLORNAME" || Val.ToString(e.Column.FieldName) == "LCOLORNAME"))
                {
                    e.Appearance.Font = new Font("Verdana", 8, FontStyle.Bold);
                    e.Appearance.BackColor = Color.FromArgb(255, 255, 192);
                }
                else if (StrClick == "CLARITY" && (Val.ToString(e.Column.FieldName) == "CLARITIY" || Val.ToString(e.Column.FieldName) == "LCLARITIY"))
                {
                    e.Appearance.Font = new Font("Verdana", 8, FontStyle.Bold);
                    e.Appearance.BackColor = Color.FromArgb(255, 255, 192);
                }
                else if (StrClick == "CUT" && (Val.ToString(e.Column.FieldName) == "CUT" || Val.ToString(e.Column.FieldName) == "LCUT"))
                {
                    e.Appearance.Font = new Font("Verdana", 8, FontStyle.Bold);
                    e.Appearance.BackColor = Color.FromArgb(255, 255, 192);
                }
                else if (StrClick == "POL" && (Val.ToString(e.Column.FieldName) == "POL" || Val.ToString(e.Column.FieldName) == "LPOL"))
                {
                    e.Appearance.Font = new Font("Verdana", 8, FontStyle.Bold);
                    e.Appearance.BackColor = Color.FromArgb(255, 255, 192);
                }
                else if (StrClick == "SYM" && (Val.ToString(e.Column.FieldName) == "SYM" || Val.ToString(e.Column.FieldName) == "LSYM"))
                {
                    e.Appearance.Font = new Font("Verdana", 8, FontStyle.Bold);
                    e.Appearance.BackColor = Color.FromArgb(255, 255, 192);
                }
                else if (StrClick == "FL" && (Val.ToString(e.Column.FieldName) == "FL" || Val.ToString(e.Column.FieldName) == "LFL"))
                {
                    e.Appearance.Font = new Font("Verdana", 8, FontStyle.Bold);
                    e.Appearance.BackColor = Color.FromArgb(255, 255, 192);
                }
                else if (StrClick == "TOTAL")
                {
                    e.Appearance.Font = new Font("Verdana", 8, FontStyle.Bold);
                    e.Appearance.BackColor = Color.FromArgb(255, 255, 192);
                }
                ...
                {
                    strcol = Val.ToString(e.Column.FieldName);
                    e.Appearance.Font = new Font("Verdana", 8, FontStyle.Regular);
        ...
        private void MainGrdTotal_Paint(object sender, PaintEventArgs e)
        {
            //GridControl gridC = sender as GridControl;
            //GridView gridView = gridC.FocusedView as GridView;
            //BandedGridViewInfo info = (BandedGridViewInfo)gridView.GetViewInfo();
