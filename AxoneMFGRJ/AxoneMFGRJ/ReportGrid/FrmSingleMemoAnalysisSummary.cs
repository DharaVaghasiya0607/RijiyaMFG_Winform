...
{
    public partial class FrmSingleMemoAnalysisSummary : Form
    {
        ...
        public FrmSingleMemoAnalysisSummary()
        {
            InitializeComponent();
        }
        ...
        private void BtnRefresh_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (DTPCompleteFromDate.Checked)
                    StrFromDate = Val.SqlDate(DTPCompleteFromDate.Value.ToShortDateString());
                else
                    StrFromDate = null;


                if (DTPCompleteToDate.Checked)
                    StrToDate = Val.SqlDate(DTPCompleteToDate.Value.ToShortDateString());
                else
                    StrToDate = null;
                ...
                DTabSummary = ObjView.GetDataForSinglMemoAnalysisForSummary(Val.Trim(CmbKapan.Properties.GetCheckedItems()), StrFromDate, StrToDate);
        ...
        private void GrdDet_RowCellClick(object sender, RowCellClickEventArgs e)
        {
            try
            {
                if (e.Column.FieldName == "KAPANNAME" && e.Clicks == 2)
                {
        ...
        private void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            {
                DataRow DRKp = GrdDet.GetFocusedDataRow();
                DSMemoDetail = ObjView.GetDataForSinglMemoAnalysis(Val.ToString(DRKp["KAPANNAME"]), 0, Val.Val(DRKp["SALERATE"]), Val.Val(DRKp["PURCHASERATE"]), Val.Val(DRKp["EXTRARATE"]), Val.Val(DRKp["ONOUTRATE"]), Val.ToString(DRKp["MEMOREPORTNOTE"]), Val.ToInt(DRKp["OTHERPRDTYPE_ID"]), "GENERATE");
            }
            catch (Exception ex)
            {
