...
namespace AxoneMFGRJ.View
{
    public partial class FrmProcessWiseReport : Form
    {
        ...
        private void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                DS = Obj.GetProcessWiseReport(Val.Trim(CmbKapan.Properties.GetCheckedItems()), mStrProcess, Val.ToInt64(txtEmployee.Tag), StrFromDate, StrToDate, IntIssueWise, BoolNotInProcess, mMainManager_ID, mStrDepartment,mStrRequiredProcess);
            }
            catch (Exception Ex)
            {
