    ...
    public class BOTRN_SingleIssueReturn
    {
        ...
        public TrnSingleIssueReturnProperty TransferGoods(TrnSingleIssueReturnProperty pClsProperty)
        {
            try
            {
                ...
                Ope.AddParams("TRANSIP", Config.ComputerMACID, DbType.String, ParameterDirection.Input);
               ...
               // Ope.AddParams("PERCENTAGE", pClsProperty.PERCENTAGE, DbType.String, ParameterDirection.Input);//Gunjan : 17-02-2023
                ...
                ArrayList AL = Ope.ExeNonQueryWithOutParameter(Config.ConnectionString, Config.ProviderName, "Trn_SingleIssueReturnSave", CommandType.StoredProcedure);
                //ArrayList AL = Ope.ExeNonQueryWithOutParameter(Config.ConnectionString, Config.ProviderName, "Trn_SingleIssueReturnSave_12042022", CommandType.StoredProcedure);
                ...
                if (AL.Count != 0)
                {
                    ...
                    pClsProperty.ReturnValueEmpRet_TRNID = Val.ToString(AL[6]);
        ...
        public DataTable PopupJangedForPrint(string pStrFormType, Int64 pIntJangedNo = 0, string pStrDate = null, string pStrOpe = "")
        {
        ...
        public DataSet GetPendingConfirmationData(Double pDouCarat)
        {
            //ObjFile.Log(new StackTrace().GetFrame(0).GetMethod().Name, "", "\t Start", BLL.GlobalDec.gStrComputerIP);
            Ope.ClearParams();
            DataSet DS = new DataSet();

            Ope.AddParams("CARAT", pDouCarat, DbType.Int64, ParameterDirection.Input);
            Ope.AddParams("COMPANY_ID", Config.gEmployeeProperty.COMPANY_ID, DbType.Int64, ParameterDirection.Input);
            Ope.AddParams("DEPARTMENT_ID", Config.gEmployeeProperty.DEPARTMENT_ID, DbType.Int32, ParameterDirection.Input);
            Ope.AddParams("EMPLOYEE_ID", Config.gEmployeeProperty.LEDGER_ID, DbType.Int64, ParameterDirection.Input);
        ...
        */
        ...
        public DataTable CheckPacketProcessReturnValidation(string pStrCheckPacketsListXML) //Add : Pinali : 20-08-2020 : 
        {
            Ope.ClearParams();
            DataTable DTab = new DataTable();
            Ope.AddParams("XMLISSUEPACKETSLIST", pStrCheckPacketsListXML, DbType.Xml, ParameterDirection.Input);
        ...
        public int GetMaxDeptJangedNoStr(string strDate) //#K : 19-10-2022
        {
            string Query = "";
            DataTable Dtab = new DataTable();
            ...
            Query = "Select Max(DEPTJANGEDNO) as DEPTJANGEDNO From TRN_SingleIssueReturn With(Nolock) Where Convert(Date,CONFDATE) = '" + strDate + "' And Isnull(DEPTJANGEDNO,0) <> 0";

            Ope.FillDTab(Config.ConnectionString, Config.ProviderName, Dtab, Query, CommandType.Text);

            if (Dtab.Rows != null)
            {
                IntDeptJangedNo = Val.ToInt(Dtab.Rows[0]["DEPTJANGEDNO"]);
            }
            else
            {
                IntDeptJangedNo = -1;
