...
namespace BusLib.Rapaport
{
    public class BOFindRap
    {
        ...
        public DataRow GetPacketDataRow(string pStrKapanName, int pIntPacketNo, string pStrTag, string pStrBarcodeNo = "",string pStrSrNoKapanName = "", int pIntPktSerialNo = 0)
        {
            ...
            Str = Str + " WHERE (KapanName = '" + pStrKapanName + "' And PacketNo = " + pIntPacketNo + " And Tag = '" + pStrTag + "') Or Barcode = '" + pStrBarcodeNo + "' Or (KapanName = '" + pStrSrNoKapanName + "' And PktSerialNo = " + pIntPktSerialNo + ")";
        ...
        */
        ...
        public Trn_RapSaveProperty Save(Trn_RapSaveProperty pClsProperty)
        {
            try
            {
                ...
                ArrayList AL = Ope.ExeNonQueryWithOutParameter(Config.ConnectionString, Config.ProviderName, "Trn_SinglePrdSave", CommandType.StoredProcedure);
                if (AL.Count != 0)
                {
                    pClsProperty.ReturnValue = Val.ToString(AL[0]);
                    pClsProperty.ReturnMessageType = Val.ToString(AL[1]);
