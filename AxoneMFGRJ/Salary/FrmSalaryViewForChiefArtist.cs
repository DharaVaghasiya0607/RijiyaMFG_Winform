...
{
    public partial class FrmSalaryViewForChiefArtist : Form
    {
        ...
        public FrmSalaryViewForChiefArtist()
        {
            InitializeComponent();
        }
        ...
        private void BtnRefresh_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                /*
                DataSet DS = Obj.GetSalaryViewSummaryDetailForChiefArtist(Val.Trim(CmbKapan.Properties.GetCheckedItems()), Val.ToInt32(txtFromPacketNo.Text), Val.ToInt32(txtToPacketNo.Text), 531, Val.ToInt64(txtEmployee.Tag), Val.SqlDate(DTPFromDate.Value.ToShortDateString()), Val.SqlDate(DTPToDate.Value.ToShortDateString()), StrShapeType, Val.ToBoolean(ChkISWithRejectedEmp.Checked));
        ...
        private void toolTipController1_GetActiveObjectInfo(object sender, DevExpress.Utils.ToolTipControllerGetActiveObjectInfoEventArgs e)
        {
            ...
            if (e.SelectedControl == MainGridSummary)
            {
                ...
                try
                {
                    ...
                    else if (hi.HitTest == GridHitTest.RowCell && hi.Column.FieldName == "MANAGERCODE")
                    {
        ...
        private void lblSaveLayoutSummary_Click(object sender, EventArgs e)
        {
            ...
            GrdDetYearMonth.Columns["MANAGERCODE"].ClearFilter();

            Stream strYearMonth = new System.IO.MemoryStream();
            GrdDetYearMonth.SaveLayoutToStream(strYearMonth);
            strYearMonth.Seek(0, System.IO.SeekOrigin.Begin);
            StreamReader readerYearMonth = new StreamReader(strYearMonth);
            string textYearMonth = readerYearMonth.ReadToEnd();

            int IntResYearMonth = new BOTRN_KapanCreate().SaveGridLayout(this.Name, GrdDetYearMonth.Name, textYearMonth);
            if (IntResYearMonth != -1)
            {
                Global.Message("Layout Successfully Saved");
            }
            GrdDetYearMonth.Columns["MANAGERCODE"].FilterInfo = new DevExpress.XtraGrid.Columns.ColumnFilterInfo("MANAGERCODE='" + Val.ToString(GrdSummary.GetRowCellValue(GrdSummary.FocusedRowHandle, "MANAGERCODE")) + "'");
        }
        ...
        private void lblDeleteLayoutSummary_Click(object sender, EventArgs e)
        {
            ...
            IntRes = new BOTRN_KapanCreate().DeleteGridLayout(this.Name, GrdDetYearMonth.Name);
        ...
        private void GrdDetail_RowClick(object sender, RowClickEventArgs e)
        {
            try
            {
                ...
                else if (e.Clicks == 2 && e.RowHandle >= 0)
                {
                    ...
                    FrmFactoryLabourDetail.ShowForm(StrKapan, IntPacketNO, StrTag, StrPacketID, StrWorkerName, IntWorkerID, "WORKER");
        ...
        private void GrdSummary_RowCellClick(object sender, RowCellClickEventArgs e)
        {
            ...
            {
                this.Cursor = Cursors.WaitCursor;
                GrdDetail.Columns["MANAGERCODE"].ClearFilter();
                GrdDetail.Columns["MANAGERCODE"].FilterInfo = new DevExpress.XtraGrid.Columns.ColumnFilterInfo("MANAGERCODE='" + Val.ToString(GrdSummary.GetRowCellValue(e.RowHandle, "MANAGERCODE")) + "'");
        ...
        private void GrdSummary_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            try
            {
                if (e.FocusedRowHandle >= 0)
                {
                    ...
                    this.Cursor = Cursors.WaitCursor;
                    GrdDetYearMonth.Columns["MANAGERCODE"].ClearFilter();
                    GrdDetYearMonth.Columns["MANAGERCODE"].FilterInfo = new DevExpress.XtraGrid.Columns.ColumnFilterInfo("MANAGERCODE='" + Val.ToString(DRow["MANAGERCODE"]) + "'");

                    string Str = "Year Month Wise Salary Summary Of Manager : '" + Val.ToString(DRow["MANAGERCODE"]) + "'";
        ...
        private void GrdDetYearMonth_RowCellClick(object sender, RowCellClickEventArgs e)
        {
            if (e.Clicks == 2 && e.RowHandle >= 0)
            {
                ...
                GrdDetail.Columns["MANAGERCODE"].FilterInfo = new DevExpress.XtraGrid.Columns.ColumnFilterInfo("MANAGERCODE='" + Val.ToString(GrdDetYearMonth.GetRowCellValue(e.RowHandle, "MANAGERCODE")) + "' AND YYMMM='" + Val.ToString(GrdDetYearMonth.GetRowCellValue(e.RowHandle, "YYMMM")) + "'");
                xtraTabControl1.SelectedTabPageIndex = 1;
                this.Cursor = Cursors.Default;
            }
        }
        ...
        private void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                DSDetail = Obj.GetSalaryViewSummaryDetailForChiefArtist(mStrKapanName, mIntFromPacketNo, mIntToPacketNo, 531, mIntEmployee_ID, Val.SqlDate(mStrFromDate), Val.SqlDate(mStrToDate), mStrShapeType, mISWithRejectedEmp);
