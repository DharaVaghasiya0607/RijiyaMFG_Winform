    ...
    public class BOMST_FormPermission
    {
        ...
        public int Save(Int64 pIntEmployeeID, DataTable DTab, DataTable pDTabDisplay, DataTable pDTabTransfer, DataTable pDTabProcess, DataTable pDTabReport, EmployeeActionRightsProperty pClsProperty)
        {
            ...
            try
            {
                ...
                Ope.AddParams("RAPDELETEPREDICTION", pClsProperty.RAPDELETEPREDICTION, DbType.Boolean, ParameterDirection.Input);
                ...
                Ope.AddParams("ISUPLOAD", pClsProperty.ISUPLOAD, DbType.Boolean, ParameterDirection.Input);
                Ope.AddParams("ISDELETE", pClsProperty.ISDELETE, DbType.Boolean, ParameterDirection.Input);
                ...
                Ope.AddParams("FILETRANSFERUPLOADPATH", pClsProperty.FILETRANSFERUPLOADPATH, DbType.String, ParameterDirection.Input);
                Ope.AddParams("FILETRANSFERDOWNLOADPATH", pClsProperty.FILETRANSFERDOWNLOADPATH, DbType.String, ParameterDirection.Input);
                Ope.AddParams("IS_FILRTRANSFERALLOW", pClsProperty.ISFILETRANSFER, DbType.Boolean, ParameterDirection.Input);
        ...
        public EmployeeActionRightsProperty EmployeeActionRightsGetDataByPK(Int64 pIntEmployeeID)
        {
            ...
            Property.ISFILETRANSFER = false;
            Property.FILETRANSFERUPLOADPATH = "";
            Property.FILETRANSFERDOWNLOADPATH = "";

            Property.ISDOLLARLOCK = false;

            Property.ISUPLOAD = false;
            ...
            if (DRow != null)
            {
                ...
                Property.FILETRANSFERUPLOADPATH = Val.ToString(DRow["FILETRANSFERUPLOADPATH"]);
                Property.FILETRANSFERDOWNLOADPATH = Val.ToString(DRow["FILETRANSFERDOWNLOADPATH"]);
                Property.ISFILETRANSFER = Val.ToBoolean(DRow["IS_FILETRANSFERALLOW"]);

                Property.ISUPLOAD = Val.ToBoolean(DRow["ISUPLOAD"]);
                Property.ISDELETE = Val.ToBoolean(DRow["ISDELETE"]);
            }
            return Property;
        }
        ...
        public string GetMessage()
        {
            ...
            string StrRes =  Ope.ExeScal(Config.ConnectionString, Config.ProviderName, Str, CommandType.Text);

            return StrRes;

        }

        public string GetExeUpdateMessage(string pStrExeVersion)
        {
            Ope.ClearParams();
            string StrRes = @"";
       
            Ope.ClearParams();
            string Str = @"Select 'Please Close The Exe , New Updates Upload In A Server' MSG FROM MST_Setting With(NoLock) Where SETTINGKEY = 'ExeVersion' And SETTINGVALUE<>'" + pStrExeVersion + "'";
            DataRow DRow = Ope.GetDataRow(Config.ConnectionString, Config.ProviderName, Str, CommandType.Text);
            if (DRow != null)
            {
                StrRes = StrRes + Val.ToString(DRow[0]);
            }
            return StrRes;
